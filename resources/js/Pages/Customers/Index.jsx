            ...
            <div className="space-y-6 lg:grid lg:grid-cols-3 lg:gap-6 lg:space-y-0">
                <section className="ta-card p-5">
                    ...
                    <form className="space-y-2" onSubmit={(e) => { e.preventDefault(); createForm.post(route('customers.store'), { onSuccess: () => createForm.reset() }); }}>
                        <div><input className="ta-input" placeholder="Name" value={createForm.data.name} onChange={(e) => createForm.setData('name', e.target.value)} required />{fieldError(createForm, 'name')}</div>
                        <div><input className="ta-input" placeholder="Phone" value={createForm.data.phone} onChange={(e) => createForm.setData('phone', e.target.value)} required />{fieldError(createForm, 'phone')}</div>
                    ...
                    {selectedCustomer && (
                        <>
                            ...
                            <form className="grid gap-3 md:grid-cols-2" onSubmit={(e) => { e.preventDefault(); editForm.put(route('customers.update', selectedCustomer.id)); }}>
                                ...
                                <div><input className="ta-input" value={editForm.data.phone} onChange={(e) => editForm.setData('phone', e.target.value)} required />{fieldError(editForm, 'phone')}</div>
                                <div><input className="ta-input" value={editForm.data.email} onChange={(e) => editForm.setData('email', e.target.value)} placeholder="Email" />{fieldError(editForm, 'email')}</div>
                                <div><input className="ta-input" type="date" value={editForm.data.birthday || ''} onChange={(e) => editForm.setData('birthday', e.target.value)} />{fieldError(editForm, 'birthday')}</div>
                                <div className="md:col-span-2"><input className="ta-input" value={editForm.data.acquisition_source || ''} onChange={(e) => editForm.setData('acquisition_source', e.target.value)} placeholder="Acquisition source" />{fieldError(editForm, 'acquisition_source')}</div>
